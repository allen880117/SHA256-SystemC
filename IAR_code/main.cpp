...
unsigned char fir_param[FIR_LEN]={1,2,3,4,5,6,5,4,3,2,1};
unsigned char delay_line[FIR_LEN];
unsigned char dl_index=0;

unsigned char fir(unsigned char input){
  unsigned char i,temp=0;
  delay_line[dl_index++]=input;
  dl_index%=FIR_LEN;

  for(i=0;i<FIR_LEN;i++){
    temp+=delay_line[dl_index++]*fir_param[i];
    dl_index%=FIR_LEN;
  }
...
void main(void) {
    ...
    while(CTRL_OUT != CTRL_OUT_DONE){
      if (CTRL_OUT == CTRL_OUT_IDLE){
        ...
        CTRL_IN = (CTRL_IN_DATA_PREFIX | ((CTRL_OUT & (~0x03))>>2)); // Then Control Signal
      }
    }
    ...
    while((CTRL_OUT & 0x03) == CTRL_OUT_DONE){
      if (CTRL_OUT == CTRL_OUT_DONE){
          ...
          i++;
          if (i == 32) break;
          else CTRL_IN = (CTRL_IN_RESULT_PREFIX | i);
      }
    }
